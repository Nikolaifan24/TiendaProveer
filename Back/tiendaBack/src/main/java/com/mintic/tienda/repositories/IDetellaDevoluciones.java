...
public interface IDetellaDevoluciones extends CrudRepository<Detalledevoluciones, Long>{
    ...
    @Query(value = "select count(*) from detalle_devoluciones d INNER JOIN productos p on d.IDProducto= p.IDProductos INNER JOIN devoluciones v on d.IDDevoluciones = v.IDDevoluciones WHERE v.CodigoVenta = :CodigoVenta AND p.nombreProducto = :nombreProducto ", nativeQuery = true)
