...
public class ControladorCompras {
	...
	public String actualizarCompras(Model model, @PathVariable(name = "CodigoCompra") Long CodigoCompra) {
		...
		if (ValidacionActualizarCompras(model, ComprasEditar) == true) {
			...
			model.addAttribute("Compra", iCompras.ListarCompras());
	...
	// public String eliminarCompras(Model model, @PathVariable(name =
	...
	private boolean ValidacionCrearCompras(Model model, ComprasDto Compras) {
		...
		if (Compras.getCodigoCompra() == null) {
			model.addAttribute("mensaje", "Falta Codigo");
			return false;
		}
		if (Compras.getNitProveedor() == null) {
			model.addAttribute("mensaje", "Faltan Nit");
			return false;
		}

		if (Compras.getFechaCompra().isEmpty()) {
			model.addAttribute("mensaje", "Faltan Fecha");
			return false;
		}

		int conteo = iCompras.ContadordeCompras(Compras.getCodigoCompra());
		if (conteo == 1) {
			model.addAttribute("mensaje", "Compra repetida");
			return false;
	...
	public String crearDetalleCompras(Model model, @PathVariable(name = "CodigoCompra") Long CodigoCompra,
	...
	public String actualizarDetalleCompras(Model model, @PathVariable(name = "CodigoCompra") Long CodigoCompra,
	...
	public String eliminarUsuario(Model model, @PathVariable(name = "codigoCompra") Long CodigoCompra,
			@PathVariable(name = "nombreProducto") String nombreProducto) {
